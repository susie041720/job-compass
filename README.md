...
一个中文个人求职管理网站，用于集中保存岗位、追踪投递、整理资料，并基于已确认的真实经历制作岗位定制简历。个人版可在本地完整使用；公开作品站使用只读虚构数据，避免暴露真实简历和求职记录。AI 是可选能力；不配置 API Key 时，手动录入、批量表格导入和日常管理仍可使用。
...
- 桌面端和手机端自适应中文界面
- 本地持久化数据库，刷新后数据仍存在
- 公开作品展示模式：只读虚构数据、服务端写入保护、禁用上传和 AI 调用
...
默认从终端显示的本地地址打开，通常是 `http://localhost:5173/`。项目不会发布到 gptsite；公开作品站使用独立的 Cloudflare Workers 与 D1，不会复制本地数据。

## 公开作品站

公开部署使用 Cloudflare Workers 与一套独立 D1 数据库。`PUBLIC_DEMO_MODE=true` 时，页面会明显标注“公开展示版”，服务端会拒绝新增、编辑、删除、上传、外部抓取和 AI 请求。访客可以浏览、筛选、预览简历示例，并在当前页面临时试改；刷新后恢复演示数据。

公开数据库只使用 [drizzle/public-demo-seed.sql](drizzle/public-demo-seed.sql) 中的虚构公司、岗位和简历。请勿把本地 `.wrangler` 数据库、真实简历、`.env` 或 API Key 上传到 GitHub。完整部署步骤见 [docs/PUBLIC_DEPLOYMENT.md](docs/PUBLIC_DEPLOYMENT.md)。
