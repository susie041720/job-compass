...
第一版支持手动填写、多段岗位文字、CSV 和 Excel。导入前会出现确认表，同一来源岗位 ID、规范化链接或公司+岗位+地点相同的岗位会被跳过。匹配分是本地可解释的估算结果，详情页会分别显示推荐理由、待确认条件和明确不符合项。

## 当前边界

- 截图提取和岗位定制 AI 需要自行配置兼容 API；未配置时会明确提示，不会生成伪结果。
- PDF/DOCX 导出使用稳定的标准模板，不承诺无损还原上传文件的原排版。
- 招聘链接暂不自动抓取需要登录的页面；请用截图或文字补充。
- 岗位发现的官方公司适配器尚未启用；当前先使用手动资料或表格导入，下一阶段接入少量公开官方来源。
