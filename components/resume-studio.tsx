...
export function ResumeStudio({ jobs, initialJob, onInitialJobHandled, onUseVersion, onCatalogChanged }: { jobs: ApplicationRecord[]; initialJob: ApplicationRecord | null; onInitialJobHandled: () => void; onUseVersion: (job: ApplicationRecord, version: Version) => Promise<void>; onCatalogChanged: () => Promise<void> }) {
  ...
  return <div><div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold tracking-[.16em] text-[#0f8b7c]">RESUME STUDIO</p><h1 className="mt-1 text-3xl font-bold tracking-tight">简历工作台</h1><p className="mt-1 text-sm text-slate-500">基础简历保留真实经历，岗位版本独立保存并可回溯</p></div><div><input ref={fileRef} type="file" accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" className="hidden" onChange={(event) => selectFile(event.target.files?.[0])}/><Button variant="outline" onClick={() => fileRef.current?.click()}><Upload/>上传基础简历</Button></div></div>
    ...
    <Tabs defaultValue="bases"><TabsList><TabsTrigger value="bases">基础简历 / 真实经历库</TabsTrigger><TabsTrigger value="versions">岗位定制版本</TabsTrigger></TabsList><TabsContent value="bases" className="pt-4"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{bases.map((item) => <article key={item.id} className="flex flex-col rounded-2xl border bg-white p-5"><div className="flex items-start justify-between"><FileText className="text-[#08796b]"/><span className="text-xs text-slate-400">{item.language}</span></div><h2 className="mt-4 font-semibold">{item.name}</h2><p className="mt-1 text-sm text-slate-500">原文件：{item.originalFileName}</p><p className="mt-2 text-xs text-slate-400">{item.versionCount} 个定制版本 · {item.applicationCount} 条投递正在使用</p><p className="mt-3 line-clamp-3 whitespace-pre-wrap text-sm leading-6 text-slate-600">{item.confirmedContent}</p><div className="mt-auto flex flex-wrap gap-2 pt-4"><Button size="sm" variant="outline" onClick={() => setPreviewBase(item)}><Eye/>查看</Button>{item.originalFileName&&<Button size="sm" variant="outline" onClick={() => openOriginal(item)}><Download/>{item.originalMime.includes("pdf")?"原文件":"下载原文件"}</Button>}<Button size="sm" variant="ghost" onClick={() => setDeleting({ kind: "base", id: item.id, title: item.name, usageCount: item.applicationCount, versionCount: item.versionCount })}><Trash2 className="text-rose-500"/>删除</Button></div></article>)}{!bases.length && <Empty text="先上传 PDF 或 DOCX，并检查解析结果。没有基础简历时，AI 不会自行生成经历。"/>}</div></TabsContent>
      <TabsContent value="versions" className="pt-4"><div className="mb-4 flex justify-end"><Button disabled={!bases.length || !jobs.length} onClick={() => { setOptimizer(true); setJobId(jobs[0]?.id || ""); }}><Sparkles/>针对岗位创建版本</Button></div><div className="grid gap-4 lg:grid-cols-2">{versions.map((version) => { const target = jobs.find((item) => item.id === version.jobId); return <article key={version.id} className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-2"><div><h2 className="font-semibold">{version.title}</h2><p className="text-sm text-slate-500">{target ? `${target.company} · ${target.position}` : "通用方向版本"} · {version.intensity}</p></div><span className="rounded-full bg-amber-50 px-3 py-1 text-xs text-amber-700">{version.status}</span></div><p className="mt-4 line-clamp-4 whitespace-pre-wrap text-sm leading-6 text-slate-600">{version.content}</p><div className="mt-4 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setPreviewVersion(version)}><Eye/>查看</Button><Button size="sm" variant="outline" onClick={() => exportDocx(version)}><Download/>DOCX</Button><Button size="sm" variant="outline" onClick={() => exportPdf(version)}><Download/>PDF</Button>{target?.applicationId && <Button size="sm" onClick={() => onUseVersion(target, version)}><Check/>设为本次投递版本</Button>}<Button size="sm" variant="ghost" onClick={() => setDeleting({ kind: "version", id: version.id, title: version.title, usageCount: version.usageCount, versionCount: 0 })}><Trash2 className="text-rose-500"/>删除</Button></div></article>; })}{!versions.length && <Empty text="选择一条岗位，生成或手动整理一份独立版本。生成后不会自动标记为已投递。"/>}</div></TabsContent></Tabs>
